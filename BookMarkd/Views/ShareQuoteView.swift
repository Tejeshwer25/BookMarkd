...
struct ShareQuoteView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if let image = self.getImageToShare() {
                ShareLink(item: image.pngData()!,
                          preview: SharePreview("Quote", image: Image(uiImage: image))) {
                    HStack {
    ...
    /// Method to get share `UIImage` for user generated quote
    /// - Returns: Shareable quote in image
    func getImageToShare() -> UIImage? {
        if let current = scrollPosition {
            let card = ShareableCardView(views: views,
                                         quote: quote,
                                         book: book,
                                         index: current).padding()
