...
struct RecommendationService {
    ...
    func generateRecommendations(from books: [BookModel]) async throws -> [RecommendedBooks] {
        // Use FoundationModels here
        let bookTitlesRead = books.map(\.title).joined(separator: ", ")
        let session = LanguageModelSession(instructions: instruction)
        let response = try await session.respond(to: "Generate 5 book titles user might love to read based on the instructions provided. The current book titles read by user are: \(bookTitlesRead)", generating: [RecommendedBooks].self)
