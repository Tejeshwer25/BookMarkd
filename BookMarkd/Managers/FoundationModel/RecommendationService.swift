...
struct RecommendationService {
    ...
    func generateRecommendations(from books: [BookModel], with preferences: [String]) async throws(FoundationModelErrors) -> [RecommendedBooks] {
        ...
        if model.availability == .unavailable(.appleIntelligenceNotEnabled) {
            ...
            throw .unavailable
        } else if model.availability == .unavailable(.modelNotReady) {
        ...
        do {
            ...
            throw .unableToGenerateResponse
        }
    }
    
    func generateBookDescriptionAndGenres(for book: BookModel) async throws(FoundationModelErrors) -> BookModel {
        if model.availability == .unavailable(.appleIntelligenceNotEnabled) {
            throw .appleIntelligenceNotEnabled
        } else if model.availability == .unavailable(.deviceNotEligible) {
            throw .unavailable
    ...
    func getBookDetailsFromBookCover(for extractedText: String) async throws(FoundationModelErrors) -> BookModel {
        if model.availability == .unavailable(.appleIntelligenceNotEnabled) {
            throw .appleIntelligenceNotEnabled
        } else if model.availability == .unavailable(.deviceNotEligible) {
            throw .unavailable
